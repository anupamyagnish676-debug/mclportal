...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // Verify the notice belongs to the requesting admin
    ...
    // Delete notice_reads first (FK constraint), then the notice itself
    await adminClient.from('notice_reads').delete().eq('notice_id', notice_id)
    const { error } = await adminClient.from('notices').delete().eq('id', notice_id)
