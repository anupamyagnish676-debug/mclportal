...
import { Shield, LogIn, LogOut, AlertTriangle, Ban, RefreshCw, ChevronLeft, ChevronRight, Filter, Lock } from 'lucide-react'
...
export default function AuditClient({ isHQ, adminArea }: Props) {
  ...
  return (
    <div className="max-w-6xl space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {!isHQ && (
              <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 border border-amber-100 text-xs font-semibold px-2 py-0.5 rounded-full">
      ...
      {/* Area restriction banner for non-HQ */}
      {!isHQ && (
        <div className="flex items-center gap-3 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
          <Lock className="w-4 h-4 text-amber-500 flex-shrink-0" />
          <p className="text-sm text-amber-800">
            You are viewing audit logs for <strong>{adminArea} Area</strong> users only.
            Contact HQ Admin to view system-wide logs.
          </p>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            {!isHQ && <p className="text-xs mt-1 text-gray-300">Showing {adminArea} Area only</p>}
          </div>
        ) : (
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-50">
            <p className="text-xs text-gray-400">
              ...
              {!isHQ && <span className="ml-1 text-amber-600">({adminArea} Area)</span>}
            </p>
